...
export const Knowledges = () => {
	...
	const [shouldProficiencySkillsShow, setShouldProficiencySkillsShow] =
		useState<boolean>(false);
	const [shouldCertificatesOpen, setShouldCertificatesOpen] =
		useState<boolean>(false);
				...
				<div className="flex max-laptop:flex-col justify-between max-laptop:gap-4">
					...
					<div className="flex flex-row max-laptop:flex-col justify-end items-center gap-8 max-laptop:gap-4">
						<div className="flex flex-row gap-4">
							<Button
								variant={typeSkill === "dev" ? "default" : "outline"}
								className={cn(
							...
							<Tooltip>
								<TooltipTrigger>
									<button
										...
										onClick={handleProficiencySkillsHelperOpen}
									>
			...
			<div className="m-2 p-2 w-[98dvw] grid grid-cols-6 max-tablet:grid-cols-2 max-laptop:grid-cols-3 gap-12 max-tablet:gap-6 max-laptop:gap-8 text-sky-900 dark:text-sky-200 border-2 border-slate-700 dark:border-slate-300 shadow-lg rounded-md">
				{filteredSkills.map((skill) => {
					return (
						...
						>
							<p className="mt-2 flex items-center justify-center text-center">
								{skill.name}
							</p>
							<Image
								alt={skill.name}
								width={96}
								height={96}
								className="w-24 max-tablet:w-12 max-laptop:w-16"
								src={skill.image}
							/>
							<Tooltip>
								<TooltipTrigger asChild>
									<div className="my-2 w-11/12 h-6 max-tablet:h-3 max-laptop:h-4 rounded-md bg-slate-700 dark:bg-slate-950">
										<div
