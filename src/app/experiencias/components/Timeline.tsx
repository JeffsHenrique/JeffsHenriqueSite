...
export const TimelineContent = ({ isPreview }: TimelineContentProps) => {
	...
	const currentExperiences = filteredExperiences.filter((experience) => {
		const isCurrent = experience.isMyCurrentExperience;
		return isCurrent;
	});
	...
	return (
		<div className="m-2 p-2 flex flex-col gap-4">
			...
			>
				{(isPreview ? currentExperiences : filteredExperiences).map(
					(experience, index) => {
