...
export const Contact = (props: ContactDialogProps) => {
	...
	const sendEmail = (event: FormEvent<HTMLFormElement>): void => {
		...
		if (emailValue && contentFormValue !== "") {
			...
			emailjs
				...
				.then(
					() => {
						...
						const emailSent = () => {
							toast.success(
								`${currentLang === "us-en" ? `The e-mail has been sent successfully!` : "O e-mail foi enviado com sucesso!"} 😄`,
								{
					...
					(error): void => {
						toast.error(
							`${currentLang === "us-en" ? `Oops, something went wrong... :(` : "Ops, deu algum erro :("}`,
							{
								position: "bottom-left",
								autoClose: 3000,
								style: {
			...
			toast.warn(
				`${currentLang === "us-en" ? `Fill in both fields, please!` : "Preencha os dois campos, por favor!"}`,
				{
					position: "bottom-left",
					autoClose: 3000,
					style: {
